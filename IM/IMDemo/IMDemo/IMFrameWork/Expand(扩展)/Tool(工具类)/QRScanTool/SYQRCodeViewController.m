...
-(void)initNavItem
{
    ...
    [RightItem setTitle:@"开灯" forState:UIControlStateNormal];
    [RightItem setTitle:@"关灯" forState:UIControlStateSelected];
    RightItem.layer.cornerRadius = 4.0f;
    RightItem.layer.masksToBounds = YES;
    [RightItem.titleLabel setFont:[UIFont systemFontOfSize:15.0f]];
    //[RightItem setBackgroundColor:_RightBgColor forState:UIControlStateNormal];
    [RightItem setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [RightItem addTarget:self action:@selector(LightBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *customRightItem = [[UIBarButtonItem alloc] initWithCustomView:RightItem];
    [RightItem setTag:1];
...
#warning 为了这个工程需要注释了解析数据正误判断------NWL
